...
export function ResolveFlagDialog({ flag, onClose }: Props) {
  ...
  // Load suggestions on dialog open (spec: "call GET /api/v1/flags/:id/suggestions on dialog open")
      ...
      <DialogContent className="max-w-md">
        ...
        {flag && (
          <div className="space-y-4">
            ...
            {/* Suggestions — always shown when available, clicking pre-fills + sets decision */}
                ...
                <div className="space-y-2">
                  {suggestions.map((s, i) => (
                    <button
                      key={i}
                      type="button"
                      className="w-full rounded border border-amber-200 bg-white p-2 text-left hover:bg-amber-50 transition-colors"
                      onClick={() => applySuggestion(s.suggested_value)}
                    >
