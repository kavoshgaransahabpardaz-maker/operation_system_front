...
import type { DocumentSummary, FieldMismatch, ProductGroupMismatch, ShipmentMismatchOut } from '@/types';
...
export function MismatchBanner({ data, documents }: Props) {
  const totalFieldConflicts = data.mismatches.length;
  const totalProductGroups = data.product_mismatches?.length ?? 0;

  if (totalFieldConflicts === 0 && totalProductGroups === 0) return null;
    ...
    <div className="rounded-xl border border-red-200 bg-background">
      <div className="flex items-center gap-2 border-b px-4 py-3">
        ...
        <h2 className="font-semibold text-sm">
          {totalFieldConflicts + totalProductGroups} conflict{totalFieldConflicts + totalProductGroups !== 1 ? 's' : ''} detected
        </h2>
        {totalErrors > 0 && (
